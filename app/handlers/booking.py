...
@router.message(Command("cancel"), StateFilter(BookingStates))
async def cancel_fsm(message: Message, state: FSMContext) -> None:
    await state.clear()
    await message.answer("❌ Бронирование отменено.", reply_markup=main_menu_kb())
...
async def confirm_booking(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    # user_id/table_id are resolved elsewhere (user registration and table
    # assignment are handled by other parts of the project) — here we only
    # assemble the fields create_reservation() needs from the FSM.
    reservation_data = {
    ...
    logger.info("Данные для create_reservation() подготовлены: %s", reservation_data)
